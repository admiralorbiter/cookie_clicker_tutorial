...
tooltips = {
    "Upgrade": "Increases cookies/sec. Costs more each time."
}
...
while running:
    ...
    mouse_x, mouse_y = pygame.mouse.get_pos()
    button_color = (0, 200, 0)  # Default green
    if (button_x <= mouse_x <= button_x + button_width and
            button_y <= mouse_y <= button_y + button_height):
        tooltip_text = font.render(tooltips["Upgrade"], True, (0, 0, 0))
        screen.blit(tooltip_text, (button_x, button_y - 50))  # Display above the button
        button_color = (0, 255, 0)  # Lighter green for hover
